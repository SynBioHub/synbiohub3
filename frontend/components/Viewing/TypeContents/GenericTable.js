...
export default function Components(properties) {
  ...
  useEffect(() => {
    ...
    else if (properties.type === 'ComponentDefinition')
      setJsonFile(componentJSON);
      ...
      for (let i = 0; i < rowPredicates.length; i++) {
        for (let j = 0; j < rowPredicates[i].length; j++) {
          if (rowPredicates[i][j].length > 1) {
            ...
            getQueryResponse(genericTableQuery, {
              uri: properties.uri,
              id: i + j,
              subquery: '',
              predicate: rowPredicates[i][j][0]
            }).then(res => {
              //console.log(res);
            });
          }
    ...
    if (components === undefined) {
      getQueryResponse(
        properties.title === 'Sequence Annotations'
          ? getLocation
          : getComponents,
        { uri: properties.uri }
      ).then(props => {
    ...
    return components.map((obj, key) => {
      if (properties.title === 'Sequence Annotations') {
        ...
        return getRow(
          ...
          {
            title: components[definitionIndex].definition,
            link: components[definitionIndex].def
          }
        );
        ...
        return getRow(
          key,
          {
            title: obj.access.replace('http://sbols.org/v2#', ''),
            link: obj.access
          },
