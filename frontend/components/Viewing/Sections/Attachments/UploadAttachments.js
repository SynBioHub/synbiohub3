...
export default function UploadAttachments(properties) {
	...
	const handleAttachURLChange = (e) => {
		...
		function updateFilledFields(name, value) {
			...
			} else if ((value === "" || value === "Attachment type...") && filledFieldsCount.includes(e.target.name)) {
	...
	return (
		<React.Fragment>
			<tr>
				...
				<td className={styles.attachtd} colSpan={2}>
					<div className={styles.attachelements}>
						<input id="attached-file-input" className={styles.fileattached} type="text" readOnly={true} />
						<label className={styles.selectfile}>
							...
							<input type="file" multiple onChange={(e) => {
								...
								if (files.length === 0) {
									...
									if (files.length > 1) document.getElementById("attached-file-input").value = `${files.length} files selected`;
									else document.getElementById("attached-file-input").value = e.target.files[0].name;
			...
			<tr>
				...
				<td colSpan={2}>
					<div type="button" className={filledFieldsCount.length === 3 ? styles.attachbutton : styles.attachbuttondisabled} onClick={() => {
						if (filledFieldsCount.length === 3) {
							const nameInput = document.getElementById("attach-url-name").value.replace(" ", "");
							const urlInput = document.getElementById("attach-url-url").value;
							const typeInput = Object.keys(edam).find(key => edam[key] === selectedOption);
							const uri = properties.uri;

							if (properties.attachments.filter(attachment => attachment.title === nameInput).length > 0) {
								...
								const attachment = {
									...
									url: urlInput,
									type: typeInput,
									uri: uri.replace("https://synbiohub.org", publicRuntimeConfig.backend)
								};

								attachFromURL(attachment);

								const convertedUrl = uri.slice(0, uri.slice(0, uri.lastIndexOf("/")).lastIndexOf("/") + 1);
								const version = "1";
								const newAttachment = {
									format: typeInput,
									size: "",
							...
							alert("You must fill out all three fields in the Lookup Attachment row.")
						}
					}}>
						Attach
					</div>
