...
public class PrepareSubmissionJob {
	...
	public PrepareSubmissionResult execute() throws SBOLValidationException, IOException, SBOLConversionException {
		System.err.println("In execute");

		if (webOfRegistries == null) {
			webOfRegistries = new HashMap<>();
		}
		if (citationPubmedIDs == null) {
			citationPubmedIDs = new ArrayList<>();
		}
		if (collectionChoices == null) {
			collectionChoices = new ArrayList<>();
		}
		if (overwrite_merge == null) {
			overwrite_merge = "";
		}
		if (tempDirPath == null) {
			tempDirPath = "";
		...
		for (String filename : attachmentFiles.keySet()) {
			...
			if (errorLog.startsWith("File is empty")) {
				...
				return outcome(false, "", log, errorLog, attachmentFiles, tempDirPath);
			}
			...
			try {
				...
				return outcome(false, "", log, error, attachmentFiles, tempDirPath);
			}
		}
		...
		return outcome(true, resultFile.getAbsolutePath(), log, errorLog, attachmentFiles, tempDirPath);

	}

	private static PrepareSubmissionResult outcome(
			boolean success,
		...
		return PrepareSubmissionResult.builder()
				.success(success)
				.resultFilename(resultFilename != null ? resultFilename : "")
				.log(log != null ? log : "")
				.errorLog(errorLog != null ? errorLog : "")
				.attachmentFiles(attachmentFiles != null ? Map.copyOf(attachmentFiles) : Map.of())
				.extractDirPath(extractDirPath != null ? extractDirPath : "")
				.build();
	}
  ...
  public void incrementallyUpdateSBOLExplorer(HashSet<String> explorerUrisToRemove, SBOLDocument topLevelsToAdd) {
    ...
    try {
      ...
      String json = gson.writeValueAsString(payload);
      String url = SBOLExplorerEndpoint.endsWith("/")
              ? SBOLExplorerEndpoint + "incrementalupdate"
              : SBOLExplorerEndpoint + "/incrementalupdate";
      HttpRequest post = HttpRequest.newBuilder()
              ...
              .build();
      System.err.println("SBOLExplorer request: " + url);

      HttpResponse<String> response = HttpClient.newHttpClient().send(post, HttpResponse.BodyHandlers.ofString());
      System.err.println("SBOLExplorer response: " + response.statusCode());
    } catch (Exception e) {
