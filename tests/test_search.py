...
class TestSearch(TestCase):
    ...
    def test_search(self):
        ...
        compare_get_request("/search/:query?", headers = headers, route_parameters = ["BBa_B00"], test_type = test_type, comparison_type="jsonlist", fields=["uri", "displayId", "version", "name", "description", "type"], key='uri')
        ...
        #"/manage" tested in test_submit.py

        #TODO: shared - Returns the meta data on objects that other users have shared with the user specified by the X-authorization token.
        ...
        compare_get_request("/public/:collectionId/:displayId/:version/subCollections", route_parameters = ["igem","categories", "1"], headers = {"Accept":"text/plain"}, test_name="subCollectionsCategories", test_type = test_type, comparison_type="jsonlist", fields=["uri", "displayId", "version", "name", "description"], key='uri')
        test_print("test_subcollections_public completed")

        #TODO: when submit works on SBH3
        ...
        #"/twins" tested in test_twins.py

        #TODO: /similar - SBOLExplorer needs to be on 
        ...
        compare_get_request("public/:collectionId/:displayId/:version/uses", test_name = "uses2", route_parameters = ["igem", "BBa_B0034", "1"],headers = {"Accept": "text/plain"}, test_type = test_type, comparison_type="jsonlist", fields=["uri", "displayId", "version", "name", "description", "type"], key='uri')
        ...
        compare_get_request("/sparql?query=:query", headers = {"Accept":"application/json"}, route_parameters = ["SELECT+%3Fsubject+%3Fpredicate+%3Fobject+WHERE+%7B+%3Fsubject+%3Fpredicate+%3Fobject+.+FILTER+%28str%28%3Fobject%29+%3D+%22BBa_B0034%22%29%7D"], test_type = test_type, comparison_type="json")
        test_print("test_sparql completed")

        #TODO: sequence search, not done in SBH3
